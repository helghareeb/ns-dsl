...
## Data availability
The headline evaluation uses **no third-party datasets**: all data are generated by the evaluation simulator under the fixed, content-hashed calibration and recorded seeds described in the Methods. The aggregated result summaries underlying every figure and table are provided as Supplementary Information and from the corresponding author on reasonable request; raw per-trial outputs will be released publicly under an open licence upon acceptance. The optional real-trace loaders under `data/` (Online Retail II, UCI, CC BY 4.0; Intel Lab Data, MIT CSAIL, open research; IEEE-CIS / ULB / PaySim fraud, Kaggle; Twitter cache trace, OSDI'20, CC BY 4.0) ship `download.sh` scripts for an out-of-scope real-trace check and are not part of the submitted headline numbers.
